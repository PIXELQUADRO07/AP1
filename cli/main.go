...
func printWithBorder(text string) {
	width := 76
	borderTop := strings.Repeat("=", width+4)
	borderBottom := borderTop

	fmt.Println(colorText(ansiCyan, borderTop))
	for _, rawLine := range strings.Split(text, "\n") {
		for _, line := range wrapText(rawLine, width) {
...
var banners = []string{
	`
	  .--------------------------------------.
	  |    AP1 Rogue AP Orchestrator CLI    |
	  |     Build successful and ready      |
	  '--------------------------------------'
	`,
	`
	  [*] Target Interface: wlp0s20f3
	  [>] AP1 CLI ready for commands
	`,
}
